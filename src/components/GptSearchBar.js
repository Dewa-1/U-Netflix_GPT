...
Task:
- If the input is an exact or partial movie title, INCLUDE that movie first.
- Then recommend similar movies.
- Return EXACTLY 5 movie titles total.

Rules:
- Include the searched movie if it exists.
- Only movie names.
- No explanations.
- No numbering.
...
`;
    ...
    try {
      ...
      const PromiseMovieArray = moviesListGPT.map((movie) =>
        searchMovieTMDB(movie),
      ); //har ek movie jo gpt se aayi hai usko tmdb me search kiya
      ...
      toast.success("Movies ready 🎬", { id: toastId, duration: 3000 });
    } catch (error) {
